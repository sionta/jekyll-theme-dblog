...
module Jekyll
  module Tags
    class MyHighlightBlock < Liquid::Block
      ...
      SYNTAX = /^([a-zA-Z0-9.+#_-]+)((\s+\w+(=(\w+|"([0-9]+\s)*[0-9]+"))?)*)$/

      def initialize(tag_name, markup, tokens)
        super
        raise SyntaxError, <<~MSG unless markup.strip =~ SYNTAX
...
Liquid::Template.register_tag('highlight', Jekyll::Tags::MyHighlightBlock)
